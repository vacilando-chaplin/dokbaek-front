...
interface FilmographyMainProps {
  ...
  onFilmoDeleteClick: React.MouseEventHandler<HTMLButtonElement>;
}
    ...
    <section className="flex h-auto w-full flex-col gap-6 rounded-2xl bg-background-surface-light p-8">
      <div className="flex w-full flex-row justify-between">
        ...
        <div className="flex items-center gap-1">
          {filmoList.length >= 1 && (
            <button className="flex h-auto w-auto items-center gap-1 rounded-[10px] border border-border-default-light bg-background-surface-light px-3 py-[7px] text-body3 font-medium leading-body3 tracking-body3 text-content-primary-light">
              대표작 설정
              <svg
                width="12"
                height="12"
      ...
      {classificationList.length >= 1 ? (
        classificationList.map((classification: string, index: number) => {
          const filmography = filmoList.filter(
